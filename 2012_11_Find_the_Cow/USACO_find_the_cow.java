...
public class USACO_find_the_cow {
    ...
    static int find_cow() {
        ...
        for (int i = 1; i < length; i++) {
            if (grass_pattern.charAt(i - 1) == '(' && grass_pattern.charAt(i) == '(') {
                // Found hind legs.
                for (int j = i + 2; j < length; j++) {
                    if (grass_pattern.charAt(j - 1) == ')' && grass_pattern.charAt(j) == ')') {
    ...
    static int find_cow_2() {
        ...
        for (int i = 1; i < grass_pattern.length(); i++) {
            ...
            else if (grass_pattern.charAt(i - 1) == ')' && grass_pattern.charAt(i) == ')') {
                // Found front legs. This pair of front legs can be combined with any other pair of hind legs to
                // form a complete cow.
                // Add number of hind legs cow counter for every pair of front legs found so far.
